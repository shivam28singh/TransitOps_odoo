<script lang="ts">
	...
	let showActiveVehicles = $derived(
		['ADMIN', 'FLEET_MANAGER', 'DISPATCHER', 'FINANCIAL_ANALYST'].includes(role)
	);
	...
	let showActiveTrips = $derived(
		['ADMIN', 'DISPATCHER', 'SAFETY_OFFICER', 'DRIVER'].includes(role)
	);
	...
	let showRecentTrips = $derived(
		['ADMIN', 'DISPATCHER', 'SAFETY_OFFICER', 'DRIVER'].includes(role)
	);
	let showVehicleStatus = $derived(
		['ADMIN', 'FLEET_MANAGER', 'DISPATCHER', 'FINANCIAL_ANALYST'].includes(role)
	);
	...
	// Options computed locally
	let regions = $derived(
		Array.from(new Set(data.vehicles.map((v) => v.region).filter(Boolean))) as string[]
	);
	...
	let filteredVehicleIds = $derived(new Set(filteredVehicles.map((v) => v.id)));
	let filteredTrips = $derived(data.trips.filter((t) => filteredVehicleIds.has(t.vehicleId)));

	// Compute KPIs dynamically
	let kpis = $derived({
		activeVehicles: filteredVehicles.filter((v) => v.status === 'ON_TRIP').length,
		availableVehicles: filteredVehicles.filter((v) => v.status === 'AVAILABLE').length,
		inMaintenanceVehicles: filteredVehicles.filter((v) => v.status === 'IN_SHOP').length,
		activeTrips: filteredTrips.filter((t) => t.status === 'IN_PROGRESS').length,
		pendingTrips: filteredTrips.filter((t) => t.status === 'SCHEDULED').length,
		driversOnDuty: data.drivers.filter((d) => d.status === 'AVAILABLE' || d.status === 'ON_TRIP')
			.length,
		fleetUtilization:
			filteredVehicles.length > 0
				? Math.round(
						(filteredVehicles.filter((v) => v.status === 'ON_TRIP').length /
	...
	// Vehicle Status counts for progress bars calculated locally from unfiltered vehicles
	let statusCounts = $derived({
		available: data.vehicles.filter((v) => v.status === 'AVAILABLE').length,
		onTrip: data.vehicles.filter((v) => v.status === 'ON_TRIP').length,
		inShop: data.vehicles.filter((v) => v.status === 'IN_SHOP').length,
		retired: data.vehicles.filter((v) => v.status === 'RETIRED').length,
	...
	<div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
		...
		<div class="text-right hidden md:block">
			<span class="text-sm font-medium"
				>{selectedRegion !== 'All' ? `Region: ${selectedRegion}` : 'All Regions'}</span
			>
		</div>
	</div>
				...
				<Select.Root type="single" bind:value={selectedType}>
					...
					<Select.Content>
						<Select.Group>
							...
							{#each vehicleTypes as type, idx (idx)}
				...
				<Select.Root type="single" bind:value={selectedRegion}>
					...
					<Select.Content>
						<Select.Group>
							...
							{#each regions as region, idx (idx)}
	...
	<div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:flex lg:flex-wrap gap-4">
		...
		{#if showActiveVehicles}
			...
			>
				...
				<div class="text-2xl font-bold tracking-tight">{kpis.activeVehicles}</div>
			</div>
		{/if}
		...
		{#if showActiveTrips}
			...
			>
				...
				<div class="text-2xl font-bold tracking-tight">{kpis.activeTrips}</div>
			</div>
		{/if}
		...
		{#if showPendingTrips}
			...
			>
				...
				<div class="text-2xl font-bold tracking-tight">{kpis.pendingTrips}</div>
			</div>
		{/if}
		...
		{#if showDriversOnDuty}
			...
			>
				...
				<div class="text-2xl font-bold tracking-tight">{kpis.driversOnDuty}</div>
			</div>
		{/if}
								...
								<Table.Row>
									...
									<Table.Cell>
										...
										{:else if trip.status === 'SCHEDULED'}
											...
											>
										{:else}
											<Badge variant="outline" class="text-muted-foreground border-muted"
												>Draft</Badge
											>
										{/if}
									</Table.Cell>
									<Table.Cell
										class="text-right text-xs text-muted-foreground truncate max-w-[150px]"
									>
							...
							{:else}
								<Table.Row>
									<Table.Cell colspan={5} class="h-24 text-center"
										>No recent trips found.</Table.Cell
									>
								</Table.Row>
							{/each}
						</Table.Body>
			...
			<div class="lg:col-span-1 bg-card border border-border rounded-xl p-6 space-y-6 shadow-xs">
				...
				<div class="space-y-4">
					{#if true}
						<!-- Available -->
						{@const avPercent =
							statusCounts.total > 0 ? (statusCounts.available / statusCounts.total) * 100 : 0}
						<div class="space-y-1.5">
							<div class="flex justify-between text-xs font-medium">
								<span>Available</span>
								<span class="text-muted-foreground"
									>{statusCounts.available} ({Math.round(avPercent)}%)</span
								>
							</div>
						...
						<!-- On Trip -->
						{@const tripPercent =
							statusCounts.total > 0 ? (statusCounts.onTrip / statusCounts.total) * 100 : 0}
						<div class="space-y-1.5">
							<div class="flex justify-between text-xs font-medium">
								<span>On Trip</span>
								<span class="text-muted-foreground"
									>{statusCounts.onTrip} ({Math.round(tripPercent)}%)</span
								>
							</div>
							<div class="h-2 w-full bg-muted rounded-full overflow-hidden">
						...
						<!-- In Shop -->
						{@const shopPercent =
							statusCounts.total > 0 ? (statusCounts.inShop / statusCounts.total) * 100 : 0}
						<div class="space-y-1.5">
							<div class="flex justify-between text-xs font-medium">
								<span>In Shop</span>
								<span class="text-muted-foreground"
									>{statusCounts.inShop} ({Math.round(shopPercent)}%)</span
								>
							</div>
							<div class="h-2 w-full bg-muted rounded-full overflow-hidden">
						...
						{@const retiredPercent =
							statusCounts.total > 0 ? (statusCounts.retired / statusCounts.total) * 100 : 0}
						<div class="space-y-1.5">
							<div class="flex justify-between text-xs font-medium">
								<span>Retired</span>
								<span class="text-muted-foreground"
									>{statusCounts.retired} ({Math.round(retiredPercent)}%)</span
								>
							</div>
							<div class="h-2 w-full bg-muted rounded-full overflow-hidden">
