			...
			>
				{#each data.monthlyRevenue as point, idx (idx)}
					<div class="flex flex-col items-center justify-end h-full w-full group relative">
						<!-- Tooltip -->
						<div
							class="opacity-0 group-hover:opacity-100 absolute -top-8 bg-slate-800 text-slate-200 text-xs py-1 px-2 rounded pointer-events-none transition-opacity whitespace-nowrap z-10"
						>
		...
		<div class="space-y-6">
			...
			<div class="space-y-5 pt-2">
				{#each data.topCostliestVehicles as vehicle, i (i)}
					<div class="flex items-center gap-4">
						...
						<div class="flex-1 h-3 bg-slate-800/50 rounded-r-sm relative overflow-hidden group">
							<div
								class="h-full rounded-r-sm shadow-inner transition-all duration-500 {vehicleColors[
									i % vehicleColors.length
								]}"
							...
							<!-- Hover Tooltip -->
							<div
								class="opacity-0 group-hover:opacity-100 absolute inset-0 flex items-center px-3 text-[10px] font-mono text-white/90 drop-shadow-md bg-black/20 pointer-events-none transition-opacity"
							>
				...
				{#if data.topCostliestVehicles.length === 0}
					<div
						class="text-slate-500 text-sm font-mono h-24 flex items-center justify-center border border-dashed border-slate-800 rounded"
					>
